...
function renderMobileCard(collection,schema,row,columns=getVisibleColumns(collection)){
  ...
  return `<article class="mobile-data-card rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition active:scale-[.99] dark:border-slate-800 dark:bg-slate-900"><div class="mobile-card-title-row">${['timeline','checklist'].includes(collection)?completionControl(collection,row,true):''}<div class="min-w-0 flex-1"><button type="button" data-detail="${esc(row.id)}" class="record-title-button text-sm font-bold leading-6">${titleDisplayValue(schema,titleKey,row[titleKey])}</button></div></div>${secondaryHtml?`<dl class="mobile-card-grid">${secondaryHtml}</dl>`:''}${showActions?`<div class="mobile-card-actions">${actionButtons(collection,row,true)}</div>`:''}</article>`;
}
...
function renderChecklistSuggestionResults(record={},useEditorValues=true){
  ...
  const refHtml=refs.length?refs.map(row=>`<div class="checklist-suggestion-row"><div class="min-w-0"><p class="text-xs font-semibold">${esc(row.group||resolveLookupLabel(String(row.group_id||''),'—'))}</p><p class="mt-0.5 text-[10px] text-slate-400">${esc(row.event||resolveLookupLabel(String(row.anchor_event_id||''),'Chưa gắn sự kiện'))} · ${Number(row.rating||0)}/5</p></div><div>${safeExternalUrl(row.sourceUrl)?`<a href="${esc(safeExternalUrl(row.sourceUrl))}" target="_blank" rel="noopener noreferrer" class="vendor-suggestion-link">Mở link tham khảo</a>`:'<span class="vendor-suggestion-link vendor-suggestion-link--empty">Chưa có đường dẫn</span>'}</div><button type="button" data-checklist-reference-detail="${esc(row.id)}" class="vendor-suggestion-action">${icon('eye','size-3.5')}<span>Xem chi tiết</span></button></div>`).join(''):'<p class="checklist-suggestion-empty">Chưa có dữ liệu Tham khảo phù hợp.</p>';
...
function renderVendorSuggestions(serviceGroup){
  ...
  if(!serviceGroup)return `<div id="vendorSuggestions" class="vendor-suggestions"><div class="vendor-suggestions__head"><div><p class="text-sm font-bold">Gợi ý tự động</p><p class="mt-1 text-[11px] text-slate-500 dark:text-slate-400">Chọn Nhóm công việc để xem gợi ý từ Tham khảo.</p></div></div></div>`;
...
function bindVendorSuggestionActions(root=document){root.querySelectorAll('[data-vendor-reference-detail]').forEach(button=>button.addEventListener('click',()=>openDetails('references',button.dataset.vendorReferenceDetail)));}
function renderVendorDetailSuggestions(record={}){
  ...
  if(!serviceGroup||!vendorSuggestionRows(serviceGroup).length)return '';
...
function openDetails(collection,id){
  ...
  const detailDialog=document.getElementById('detailDialog');if(!detailDialog.open)detailDialog.showModal();
  ...
  const detailContent=document.getElementById('detailContent');bindVendorSuggestionActions(detailContent);bindChecklistSuggestionDetailActions(detailContent);refreshIcons();
}
...
async function postAppsScript(payload,options={}){
  ...
  const body={...payload,source:'WeddingOS',clientVersion:'11.0.2',sentAt:new Date().toISOString(),requestId:payload.requestId||uid('request'),...(token?{sessionToken:token}:{}),...(!hasPayloadPassword&&attachBootstrapPassword?{password}:{}),...(!hasPayloadSchemaPassword&&schemaPassword?{schemaPassword}:{})};
