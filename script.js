  ...
  schemas: {
    checklist: {
      ...
      fields:[
        ...
        ['offsetDays','Số ngày lệch so với sự kiện','number',{backendLabel:'Offset ngày',editorHidden:true}], ['startDate','Ngày bắt đầu','date'], ['dueDate','Hạn hoàn thành','date'],
    ...
    timeline: {
      ...
      fields:[
        ['eventDate','Ngày sự kiện','date'], ['event','Tên sự kiện / hoạt động','text',{backendLabel:'Sự kiện'}], ['startTime','Giờ bắt đầu','time'], ['durationMinutes','Thời lượng','number',{backendLabel:'Thời lượng (phút)',helpText:'Đơn vị: phút'}],
    ...
    vendors: {
      ...
      fields:[
        ...
        ['score','Điểm đánh giá','number',{backendLabel:'Điểm /10',helpText:'Thang điểm 10'}], ['status','Trạng thái','select',['Đang khảo sát','Đã nhận báo giá','Vào shortlist','Đã chọn','Đã cọc','Hoàn tất','Loại']],
      ...
      sections:[
        ...
        {id:'service',title:'Dịch vụ & báo giá',icon:'receipt-text',fields:['quote','includes']},
        {id:'contract',title:'Thanh toán & hợp đồng',icon:'file-signature',fields:['deposit','paymentTerms','contractUrl']},
        {id:'decision',title:'Đánh giá & quyết định',icon:'badge-check',fields:['score','status','decisionDue']},
        {id:'notes',title:'Ghi chú & tài liệu',icon:'file-text',fields:['notes']}
      ],
    ...
    references: {
      ...
      sections:[
        ...
        {id:'source',title:'Nguồn tham khảo',icon:'link-2',fields:['sourceUrl','source']},
        {id:'rating',title:'Đánh giá',icon:'star',fields:['interestLevel','priorityLevel','rating']},
        {id:'notes',title:'Ghi chú & tài liệu',icon:'file-text',fields:['notes']}
      ],
...
function manifestFieldsForModule(collection,schema) {
  ...
  return ordered.filter(key=>/^[A-Za-z][A-Za-z0-9_]{0,79}$/.test(key)).map(key=>{
    ...
    const field={key,label:manifestFieldLabel(key,options?.backendLabel||definition?.[1]),type,required:key==='id',hidden:key==='id'||key==='updatedAt',allowBlank:key!=='id',width:manifestFieldWidth(type,key)};
...
function buildSchemaManifest() {
  ...
  modules.lookups={collection:'lookups',sheetName:'lookups',title:'Danh mục dùng chung',dataShape:'lookupMap',sensitive:false,adminOnly:false,ownerScoped:false,fields:[
...
function renderAdminLockedSettingsCard(title,description,iconName='shield-keyhole'){
  return `<section class="rounded-3xl border border-slate-200 bg-white shadow-soft dark:border-slate-800 dark:bg-slate-900"><div class="flex flex-col items-center px-6 py-8 text-center"><span class="grid size-12 place-items-center rounded-2xl bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-300">${icon(iconName,'size-5')}</span><h3 class="mt-4 font-bold tracking-tight">${esc(title)}</h3><p class="mt-1 max-w-md text-xs leading-5 text-slate-500 dark:text-slate-400">${esc(description)}</p><button type="button" data-settings-admin-unlock="1" class="mt-4 inline-flex h-10 items-center gap-2 rounded-xl bg-slate-900 px-4 text-xs font-semibold text-white transition hover:bg-brand-700 dark:bg-white dark:text-slate-950 dark:hover:bg-brand-300">${icon('unlock-keyhole','size-4')}Nhập mật khẩu quản trị</button></div></section>`;
}
...
function openEditor(collection,id='',mode='edit'){
  ...
  bindNumberInputs(fields);bindReferenceSourceDetection(fields,collection);bindAttachmentEditorControls();
...
function renderEditorField([key,label,type,options],value=''){
  ...
  const wide=['textarea'].includes(type)||['task','description','notes','includes','paymentTerms','contractUrl','sourceUrl'].includes(key);
...
function bindPageEvents(){
  ...
  document.getElementById('settingsForm')?.addEventListener('submit',saveSettingsForm); document.getElementById('settingsThemeButton')?.addEventListener('click',toggleTheme); document.querySelectorAll('[data-settings-admin-unlock]').forEach(button=>button.addEventListener('click',openSettingsAccessDialog));
...
async function postAppsScript(payload,options={}){
  ...
  const body={...payload,source:'WeddingOS',clientVersion:'9.6.3',sentAt:new Date().toISOString(),requestId:payload.requestId||uid('request'),...(token?{sessionToken:token}:{}),...(!hasPayloadPassword&&attachBootstrapPassword?{password}:{}),...(!hasPayloadSchemaPassword&&schemaPassword?{schemaPassword}:{})};
